...
def train(model, device, loader_train, optimizer, epoch):
    ...
    for i in range(len(arr_data[0])):
        ...
        loss = loss_fn(output, y) #不需要float32
        loss = torch.sum(loss)
        loss.backward()
        optimizer.step()
...
data_train = TestbedDataset(root='data', dataset='esol_train')
data_valid = TestbedDataset(root='data', dataset='esol_validation')
data_test = TestbedDataset(root='data', dataset='esol_test')
