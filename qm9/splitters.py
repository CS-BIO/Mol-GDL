...
class ScaffoldSplitter(Splitter):
    ...
    def split(self, 
        ...
        train_dataset = list(np.array(dataset,dtype=object)[train_idx])
        valid_dataset = list(np.array(dataset,dtype=object)[valid_idx])
        test_dataset = list(np.array(dataset,dtype=object)[test_idx])
        return train_dataset, valid_dataset, test_dataset
...
class RandomScaffoldSplitter(Splitter):
    ...
    def split(self, 
        ...
        for ind in range(N):
            scaffold = generate_scaffold(dataset[ind][0], include_chirality=True)
